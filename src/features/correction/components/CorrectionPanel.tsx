...
import { DEFAULT_CORRECTION_MODEL } from '../../../lib/correction/modelManager'
    ...
    <section className="mt-8 border-t border-[#e0e8e3] pt-6" aria-labelledby="correction-heading">
      ...
      <div className="mt-5 grid gap-4 rounded-[12px] border border-[#d8e1dc] bg-[#f7faf7] p-4 md:grid-cols-2 md:p-5">
        <div className="text-xs text-[#71807b]">
          ...
          <p className="mt-2 font-mono text-[11px] text-[#1d6b50]">{DEFAULT_CORRECTION_MODEL.label}</p>
          <p className="mt-1 text-[10px] text-[#9aa6a1]">
            初回のみモデルをダウンロードします（約
            {formatModelSize(DEFAULT_CORRECTION_MODEL.totalSizeBytes)}）。
          </p>
        </div>
        <div className="text-xs text-[#71807b]">
