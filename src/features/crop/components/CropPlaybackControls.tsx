...
export function CropPlaybackControls({
  ...
  return (
    <div className="relative mt-2 flex flex-wrap items-center gap-3 px-3.5 pb-9 pt-4">
      ...
      <div className="min-w-[220px] flex-1 translate-y-1">
        ...
        >
          ...
          <div
            className="pointer-events-none absolute top-1/2 h-2 -translate-y-1/2 rounded-full bg-[#79aa93] shadow-[inset_0_0_0_1px_rgba(23,77,60,0.3)]"
            style={{
          ...
          {(['start', 'end'] as const).map((handle) => {
            ...
            return (
              ...
              >
                <span className="absolute bottom-9 left-1/2 -translate-x-1/2 whitespace-nowrap rounded bg-[#f4f7f4]/95 px-1 py-0.5 font-mono text-[9px] font-medium text-[#174d3c]">
                  {formatTrimTime(timestampMs)}
                </span>
                <span className="absolute left-1/2 top-1/2 h-7 w-1.5 -translate-x-1/2 -translate-y-1/2 rounded-full border border-[#174d3c] bg-[#f3faf6] shadow-[0_1px_3px_rgba(22,54,42,0.22)]" />
              </button>
            )
          })}
      ...
      <button
        className="inline-flex shrink-0 translate-y-1 items-center gap-1.5 rounded-md px-2 py-2 text-[10px] font-semibold text-[#71807b] transition hover:bg-[#e2eee8] hover:text-[#174d3c] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1d6b50]/30 disabled:cursor-not-allowed disabled:opacity-50"
        type="button"
        onClick={onTrimReset}
        disabled={isApplying}
        title="カット範囲を全範囲に戻す"
      ...
      <p className="pointer-events-none absolute bottom-2 left-3.5 text-[10px] leading-4 text-[#71807b]">
        左右のつまみで解析範囲を指定できます。緑部分を解析対象とし、グレー部分は解析から除外されます。
      </p>
    </div>
  )
}
